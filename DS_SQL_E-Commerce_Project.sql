...
wE are asked to make the following analyzes for this scenario by following the
instructions given.
...
-- I use distinct because there are so many repeated value of Same order_id
select top 3 mf.Cust_id,cd.Customer_Name, count(distinct ord_id) as Count_orders
from market_fact mf
inner join cust_dimen cd on mf.Cust_id = cd.Cust_id
...
-- way 3
select distinct Prod_id, Sum(Sales) total_sales
from market_fact
group by Prod_id 
...
count(Ship_Date) OVER (PARTITION BY month(Ship_Date) order by month(Ship_Date)) AS Total_Unique_Customers 
FROM combined_table 
...
AND cust_id IN 
			...
			FROM combined_table 
			WHERE year(Ship_Date) = 2011 AND month(Ship_Date) = 06
			);
